pipeline {
    ...
    stages{
                    ...
                    withSonarQubeEnv('EFS_Sonar') {
                        withMaven(
                           mavenSettingsConfig: '5b6df7e0-ccd7-4cf8-87f1-281c1eebc89f'
                            ...
                            sh "mvn test"
                            sh "mvn clean verify sonar:sonar -Dsonar.token=$SONARTOKEN -Dsonar.host.url=https://sonarqube.efs-techhub.com -Dsonar.projectKey=metadata"
                        }
                    }
                }
            }
        ...
        stage('upload artefact') {
            steps {
                container('maven') {
                    sh 'curl -v -X "POST" "https://dtrack-api.efs-techhub.com/api/v1/bom" -H "Content-Type:multipart/form-data" -H "X-Api-Key: $APIKEY" -F "project=3aceb718-11a3-4e36-b014-cddbe1020563" -F "bom=@result.json"'
                }
            }
        }
